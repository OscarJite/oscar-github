...
const RepoCard = () => {
  ...
  const repoName = params.get('name')
   ...
   const FetchAPIFromServer = async () => {
      const response = await fetch(`https://api.github.com/repos/oscarjite/${repoName}`);
      ...
      useEffect(() => {
        const FetchAPIFromServer = async () => {
      const response = await fetch(`https://api.github.com/repos/oscarjite/${repoName}/commits`);
      const result = await response.json();
      ...
      useEffect(() => {
        const FetchAPIFromServer = async () => {
      const response = await fetch(`https://api.github.com/repos/oscarjite/${repoName}/languages`);
      const result = await response.json();
